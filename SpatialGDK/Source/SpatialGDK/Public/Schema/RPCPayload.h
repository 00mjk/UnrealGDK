...
struct RPCPayload
{
	...
	RPCPayload(uint32 InOffset, uint32 InIndex, uint64 InId, TArray<uint8>&& Data, TraceKey InTraceKey = InvalidTraceKey)
		: Offset(InOffset)
		, Index(InIndex)
		, Id(InId)
	...
	RPCPayload(Schema_Object* RPCObject)
	{
		...
		Id = Schema_GetUint32(RPCObject, SpatialConstants::UNREAL_RPC_PAYLOAD_RPC_ID);
									...
									int32 NumElems)
	{
		...
		Schema_AddUint32(RPCObject, SpatialConstants::UNREAL_RPC_PAYLOAD_RPC_ID, UniqueId);
...
struct RPCsOnEntityCreation : Component
{
	...
	Worker_ComponentData CreateRPCPayloadData() const
	{
		...
		for (const auto& Payload : RPCs)
		{
			...
			RPCPayload::WriteToSchemaObject(Obj, Payload.Offset, Payload.Index, Payload.Id, Payload.PayloadData.GetData(),
