...
void CrossServerRPCHandler::ProcessMessages(const TArray<Worker_Op>& WorkerMessages)
{
...
void CrossServerRPCHandler::ProcessPendingCrossServerRPCs()
{
	for (auto CrossServerRPCs = QueuedCrossServerRPCs.CreateIterator(); CrossServerRPCs; ++CrossServerRPCs)
	{
		...
		for (const auto& Command : CrossServerRPCs.Value())
		{
			...
			RPCGuidsInFlight.Remove(Command.Payload.Id);
			RPCsToDelete.HeapPush(TTuple<FDateTime, uint32>(FDateTime::Now() + Command.TimeoutMillis, Command.Payload.Id));
			++ProcessedRPCs;
		}
...
void CrossServerRPCHandler::HandleWorkerOp(const Worker_Op& Op)
{
	...
	if (RPCGuidsInFlight.Contains(Params.GetValue().Payload.Id) || RPCsToDelete.ContainsByPredicate([&](TTuple<FDateTime, uint32> Result) {
	...
	RPCGuidsInFlight.Add(Params.GetValue().Payload.Id);
