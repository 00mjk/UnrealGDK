...
TOptional<FCrossServerRPCParams> RPCExecutor::TryRetrieveCrossServerRPCParams(const Worker_Op& Op)
{
	...
	const FSpatialGDKSpanId SpanId =
		NetDriver->Connection->GetEventTracer()->TraceEvent(FSpatialTraceEventBuilder::CreateReceiveCommandRequest(
			"RPC_COMMAND_REQUEST", TargetActor, TraceTargetObject, Function, TraceId, Op.op.command_request.request_id));
	FCrossServerRPCParams Params(ObjectRef, Op.op.command_request.request_id, MoveTemp(Payload), Op.op.command_request.timeout_millis,
								 SpanId);
